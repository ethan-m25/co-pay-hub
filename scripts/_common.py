#!/usr/bin/env python3
"""
co-pay-hub/scripts/_common.py
Shared utilities for all CO Pay Hub search scripts.

CO Equal Pay for Equal Work Act (EPEWA), C.R.S. § 8-5-201 et seq.
Effective Jan 1, 2021 (enhanced Jan 1, 2023): all employers must disclose
pay rate/range + general description of benefits + other compensation.
Strictest US salary transparency law — "DOE" listings explicitly violate EPEWA.
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"co-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/co-pay-hub/data/jobs.json")
...
SKIP_PATTERNS = [
    ...
    "builtincolorado.com/salaries",  # salary guide pages, not job postings
]
...
_JOB_PAGE_MARKERS = [
    ...
    "equal pay for equal work",  # CO-specific marker
]
...
CO_TERMS = [
    ...
    "highlands ranch", "littleton", "centennial", "pueblo", "greeley",
    "longmont", "broomfield", "castle rock", "englewood", "golden",
    "parker", "loveland", "durango", "vail", "aspen", "steamboat",
    ", co", "co,",  # "City, CO" / "CO, USA"
...
EXTRACT_PROMPT = """\
Extract ONE Colorado job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Colorado job with explicit USD salary range is found:
{{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Denver, CO","source_url":"{url}","posted":"YYYY-MM-DD"}}
...
- Not a Colorado location (Denver, Boulder, Fort Collins, Colorado Springs, remote CO, etc.)
...
- location must be in Colorado (Denver, Boulder, Fort Collins, Colorado Springs, Aurora, or remote roles that mention CO pay range)
...
def extract_job(url, snippet, page_text, log=None):
    ...
    if not any(t in job.get("location", "").lower() for t in CO_TERMS):
        return None
