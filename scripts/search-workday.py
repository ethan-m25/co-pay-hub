#!/usr/bin/env python3
"""
co-pay-hub/scripts/search-workday.py
Workday API scraper — Colorado edition.

CO Equal Pay for Equal Work Act (effective Jan 1, 2021)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Colorado employers
  2. CXS JSON API   — paginate all jobs per employer, filter Colorado (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/co-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/co-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/co-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Colorado"
...
# ── Seed tenants ─────────────────────────────────────────────────────────────
SEED_TENANTS = [
    ("davita.wd1.myworkdayjobs.com", "davita", "DaVitaCareers", "DaVita"),
    ...
    ("molsoncoors.wd5.myworkdayjobs.com", "molsoncoors", "External", "Molson Coors"),
    ("vailresorts.wd5.myworkdayjobs.com", "vailresorts", "VailResortsCareers", "Vail Resorts"),
    ("newmont.wd5.myworkdayjobs.com", "newmont", "External", "Newmont Mining"),
...
KNOWN_COMPANY_OVERRIDES = {
    ...
    "boeing": "Boeing",
    "molsoncoors": "Molson Coors",
    "vailresorts": "Vail Resorts",
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Colorado" salary 2026',
    'site:myworkdayjobs.com "Denver" OR "Boulder" job salary "$" 2026',
    'site:myworkdayjobs.com "Colorado Springs" OR "Fort Collins" salary 2026',
    'site:myworkdayjobs.com Colorado tech OR software OR engineering salary 2026',
    'site:myworkdayjobs.com Colorado defense OR aerospace OR healthcare salary 2026',
    'site:myworkdayjobs.com "Denver" finance OR consulting salary range 2026',
]
...
SALARY_RE = [
    ...
    # "between $90,000 and $115,000" — Morgan Stanley / pay transparency format
...
def is_colorado(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Colorado.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_colorado(p.get("locationsText", ""), p.get("externalPath", "")):
                    co_jobs.append(p)
            offset += limit
        ...
        log(f"  Colorado jobs: {len(co_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(co_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_colorado(location, external_path):
            continue
    ...
    for sitemap_url, sitemap_company in SITEMAP_SOURCES:
        ...
        if not job_urls:
            ...
            continue
        co_urls = [u for u in job_urls if any(t in u.lower() for t in
                   ["colorado", "denver", "boulder", "-co-", "aurora-colorado",
                    "colorado-springs", "fort-collins"])]
        if not co_urls:
            co_urls = job_urls
        log(f"    → {len(job_urls)} total, {len(co_urls)} plausibly CO — checking each")
        for job_url in co_urls[:50]:
            ...
            if not is_colorado(location, ext_path):
                continue
            val_min, val_max = salary
            job = {
                "role": title,
